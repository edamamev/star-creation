...
@export var ZOOM_SPEED: float = 10 # Speed use when is_zoom_in or is_zoom_out is true
...
@export var MOUSE_ZOOM_SPEED: float = 50
@export var TOUCH_INVERT_ZOOM: bool = false
@export var MIN_MAX_DISTANCE: Vector2 = Vector2(10, 50)
...
func _process_transformation(delta: float):
	...
	_rotation.x = clamp(_rotation.x, -PI/3, PI/16)
	_move_speed = Vector2()
	
	# Update distance
	_distance += _scroll_speed * delta
	_distance = clamp(_distance, MIN_MAX_DISTANCE.x, MIN_MAX_DISTANCE.y)
	_scroll_speed = 0
